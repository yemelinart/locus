...
Статус: **0.8.0, рабочая ранняя версия**, не готовый универсальный поисковик и не доказанный лидер рынка.

## Новое в 0.8.0

До работы модели выполняется короткий проход по основным написаниям имени, включая латиницу без ограничения городом. Найденные ссылки видны сразу, даже если сайт не позволяет автоматически прочитать профиль. Неподтверждённые биографии однофамильцев больше не используются как сведения об искомом человеке при общем планировании. Проверка личности остаётся отдельным этапом.

[Разбор пропущенных профилей и архитектура поиска](docs/V080-DISCOVERY.md).

## Новое в 0.7.0

Движок проверяет другой выбранный поисковик при слабой выдаче, отбирает текст страниц по вашим ориентирам и поочерёдно исследует недостающие условия. Неверный ответ модели получает ограниченное восстановление; проблемный шаг может быть отложен, пока работа продолжается по другим направлениям. Восстановлены полезные переходы к явно связанным профессиональным профилям.

[Архитектурный аудит, сравнение и границы проверки](docs/V070-QUALITY-AUDIT.md). В автоматические проверки добавлены воспроизводимые сценарии качества. Превосходство над конкурентами пока не доказано.

## Новое в 0.6.3

Одно совпадение имени больше не получает положительную шкалу и подпись «Возможное совпадение». Проверенная оценка личности управляет переходами по найденным ссылкам: сначала ограниченная проверка биографии или связанного профиля, затем расширение публичной деятельности при подтверждённых условиях.

[Где работает ИИ и как его решение влияет на поиск](docs/V063-IDENTITY-DECISIONS.md). Надёжность нахождения людей ещё требует более широких измерений.

## Новое в 0.6.2

Продолжение после исчерпания лимита предлагает бюджет следующего прохода. При прежних условиях сохраняются очередь, оценки и история посещённых страниц. Таймер обновляется каждую секунду; ход работы показывает роль локального ИИ и веб-модуля, а также результат последней проверки совпадения. Проверка недостающих связей получает приоритет перед общим поиском.

[Как работает продолжение и план развития поиска](docs/V062-CONTINUATION.md). Эти изменения исправляют управление поиском; надёжность нахождения людей в целом ещё не доказана.

## Новое в 0.6.1

Исправлена ложная остановка поиска: пустая выдача больше не считается поломкой
подключения. Локальный географический справочник проверяет соответствие города
и страны независимо от ответа модели. Улучшены варианты имён с отчеством,
порядок имени/фамилии и запросы по биографическим ориентирам. Прежние оценки
ждут перепроверки при явном продолжении поиска.
[Аудит движка и сравнение с рынком](docs/V061-SEARCH-AUDIT.md) описывают
найденные дефекты и оставшиеся ограничения. Географические данные имеют
[отдельную атрибуцию](backend/locus/geodata/README.md).
...
Поисковые запросы стали разнообразнее: имя с отдельным ориентиром, разные написания и ограниченные широкие пробы. Кириллица и латиница проверяются в обоих направлениях. Это расширяет обнаружение страниц; обязательные критерии итогового совпадения сохраняются.

Приложение сохраняет реальные ссылки между страницами и проходит по подходящим зацепкам. Если страницы с карточками связаны, оно предлагает проверить связь. После вашего явного подтверждения «один человек» сведения из разных источников могут вместе подтвердить критерии. Цитаты сохраняют исходные ссылки; противоречия, устаревшие решения и отклонённые карточки не превращаются в совпадение. Решение можно отменить, исходные карточки не склеиваются и не удаляются.

Очередь чередует поисковые направления и сайты, а временно отказывающие адаптеры получают короткую паузу. Пустые прежние страницы можно повторно анализировать после изменения критериев. Перед переходом на схему базы 5 создаётся резервная копия. Прежние страницы хранятся как старые снимки: ссылки собираются при новом чтении.

[Аудит продукта и план](docs/V06-PRODUCT-AUDIT.md) · [Архитектура движка](docs/V06-ENGINE.md)
...
Исходники не зависят от базы: установка зависимостей и новая сборка не удаляют исследования. После обновления исходников запустите `scripts/setup.sh` и затем `scripts/start.sh`. Пока нет автоматического обновления и подписанного macOS-дистрибутива. Схема базы в 0.6 — версия 5. Перед переходом со схемы 1, 2, 3 или 4 создаётся `data/locus.vN.backup.sqlite3`, где N — прежняя версия. Существующие записи и настройки сохраняются. Более новая неподдерживаемая схема не изменяется. Прежние карточки ждут смысловой перепроверки при следующем явном запуске; открытие приложения модель не запускает.
