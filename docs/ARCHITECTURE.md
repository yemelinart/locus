...
Текущее уточнение движка: [обнаружение 0.8.0](V080-DISCOVERY.md), [аудит 0.7.0](V070-QUALITY-AUDIT.md). Географическая
...
Версия 0.7.0 добавляет отдельный модуль ранжирования обнаружения, выбор фрагментов по покрытию условий, ограниченное восстановление ответа модели и равномерную проверку недостающих ориентиров. Ошибки чтения и ИИ остаются неопределённостью. Сохранённые решения пользователя и ранее проверенные карточки не пересматриваются автоматически.

Версия 0.8.0 ставит ограниченный проход по написаниям имени перед модельным анализом. `discovery.coverage_queries` задаёт пробы без обязательного города в каждом запросе; `leads.py` формирует отдельное представление обнаруженных ссылок из сохранённой очереди. `Store.discover` сохраняет поисковый фрагмент и происхождение, не повторяя прочитанные URL. В общий план не передаётся биография неподтверждённого однофамильца. Поисковые фрагменты не поступают в оценку личности как доказанные факты.
